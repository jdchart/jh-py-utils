...
#     best_stream = yt.streams.filter(only_video=True).order_by('resolution').desc().first()

#     print(best_stream)

#     return {
#         "duration" : duration_seconds,
#         "width" : 2654.8672566371683,
#         "height" : 1500
#     }

# url = "https://youtu.be/6yoq7t8AuoA?si=NOId1WvScD41ZV52"

# print(get_youtube_info(url))
...
# # List available video-only (adaptive) streams to get resolutions
# video_streams = yt.streams.filter(only_video=True).order_by('resolution').desc()

# for stream in video_streams:
#     print(f"Resolution: {stream.resolution}, FPS: {stream.fps}, Codec: {stream.codecs}, Mime type: {stream.mime_type}")


# # BEST DIMS:
# best_stream = yt.streams.filter(only_video=True).order_by('resolution').desc().first()
# print(f"Best resolution: {best_stream.resolution}")
